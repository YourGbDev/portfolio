...
export function Certifications() {
  ...
  return (
    ...
    >
      ...
      {/* 3-Column Card Grid OR Coming Soon Placeholder */}
      {featuredCerts.length === 0 ? (
        <motion.div
          variants={shouldReduceMotion ? undefined : gridItemVariants}
          className="relative p-6 sm:p-8 rounded-xl bg-surface/30 border border-dashed border-border-hairline hover:bg-surface/60 hover:border-border-hairline transition-all duration-200 group flex flex-col items-center justify-center text-center overflow-hidden shadow-2xs"
        >
          {/* Subtle decorative curve in dark mode */}
          <svg
            className="absolute right-0 top-0 w-32 h-32 pointer-events-none opacity-5 dark:opacity-15 stroke-brand transition-opacity group-hover:opacity-20"
            viewBox="0 0 100 100"
            fill="none"
          >
          ...
          <div className="relative z-10 flex flex-col items-center gap-3 text-center">
            <div className="w-12 h-12 rounded-xl bg-surface border border-border-hairline flex items-center justify-center shadow-2xs">
